...
class AqlBuilderTest {
                        ...
                        .buildScan(
                                ...
                                List.of(
                                        new ArangoColumnHandle(
                                                "name", VARCHAR, false, List.of("name"))));
    ...
    void rendersEqualityFilter() {
        ...
        AqlQuery q =
                new AqlBuilder()
                        .buildScan(
                                handleWithConstraint(
                                        ImmutableMap.of(age, Domain.singleValue(BIGINT, 30L))),
                                List.of(age));
        assertThat(q.aql())
                .isEqualTo(
                        "FOR d IN @@col FILTER (d[\"age\"] == @v0) RETURN {\"age\": d[\"age\"]}");
                ...
                new AqlBuilder()
                        .buildScan(
                                handleWithConstraint(
                                        ImmutableMap.of(
                                                age,
                                                Domain.multipleValues(BIGINT, List.of(30L, 40L)))),
                                List.of(age));
        assertThat(q.aql())
                .isEqualTo(
                        "FOR d IN @@col FILTER (d[\"age\"] IN @v0) RETURN {\"age\": d[\"age\"]}");
        assertThat(q.bindVars()).containsEntry("v0", List.of(30L, 40L));
    }

    @Test
    void bigintRangeRendersWithNumberGuardOnly() {
        // BIGINT range uses a bare IS_NUMBER guard -- NO `== FLOOR(...)` integrality conjunct. AQL
        // FLOOR()
        // returns a double, so `d.f == FLOOR(d.f)` would false-miss a stored int64 > 2^53 (review
        // finding
        // C3). It is unnecessary anyway: BIGINT range is prefilter-only, so admitting
    ...
    void doubleRangeRendersWithNumberGuardAndDoublePromotion() {
        // A DOUBLE range promotes the operand into double space with `+ 0.0` so AQL compares
        // exactly
        // what appendValue's n.doubleValue() emits (a bare comparison diverges for a stored int64 >
        // 2^53
        // -- review finding C1); the IS_NUMBER guard precedes it because `+ 0.0` coerces
        // non-numbers.
    ...
    void doubleEqualityRendersWithNumberGuardAndDoublePromotion() {
        // DOUBLE equality/IN also promotes and guards (unlike BOOLEAN/VARCHAR/BIGINT equality,
        // which is
        // type-exact in AQL and stays bare) -- otherwise a stored int64 > 2^53 would match a bind
        // it
                ...
                new AqlBuilder()
                        .buildScan(
                                handleWithConstraint(
                                        ImmutableMap.of(
                                                name,
                                                Domain.singleValue(VARCHAR, utf8Slice("ada")))),
                                List.of(name));
    ...
    void rendersMultiSegmentPathAsChainedBracketAccess() {
        ...
        assertThat(q.aql())
                .isEqualTo("FOR d IN @@col RETURN {\"address$city\": d[\"address\"][\"city\"]}");
    }
}
