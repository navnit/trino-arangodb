...
| `ArangoTableHandle` with an aggregation descriptor, non-empty `groupingColumns`, pushed limit `n` | `Estimate.of(n)`, no client call | `applyLimit` reports `limitGuaranteed=true` for aggregated handles (always a single split, `LIMIT` rendered after the `COLLECT`), so `n` is an exact output cap — the same known-upper-bound principle as the `min(count, n)` row, and equally free. |
