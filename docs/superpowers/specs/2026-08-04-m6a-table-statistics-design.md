...
| `ArangoTableHandle` with an aggregation descriptor, non-empty `groupingColumns` | `empty()` | Output rows are groups; group cardinality is unknowable without NDV stats, and the base count would be arbitrarily wrong. |
...
- Query handle → `empty()`; grouped aggregated handle → `empty()`; global aggregated handle (empty `groupingColumns`) → row count 1 with **zero client calls**.
...
- Handle carrying *both* an aggregation and a limit → the aggregation rows win (pins §2's guard ordering).
