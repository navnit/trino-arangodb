...
10. **Any domain on the handle's constraint that is `isPrefilterOnly`** (`BIGINT` + non-discrete, `ArangoMetadata:300`). Such a constraint is enforced *jointly* by the pushed AQL and Trino's residual re-check; an aggregate computed on top of the AQL side alone would include rows — fractional or out-of-long-range values in the *filter* column — that the residual would have dropped. **Implementation measured this to be load-bearing, not defense-in-depth.** The original reasoning was that Trino never offers this shape because a residual leaves a `FilterNode` that `PushAggregationIntoTableScan` will not match; in fact Trino fuses the predicate into a `ScanFilterProject` and pushes an aggregate straight through it (proven by the `DOUBLE`-range case, which is fully pushed down). Without this rule the `BIGINT`-range case would push and return a silently wrong count. See §10/8.
...
8. **A `BIGINT`-range predicate suppresses aggregation pushdown — and that suppression is load-bearing, not cosmetic.** *(Corrected 2026-07-26 during implementation; the original text here reasoned that Trino's `PushAggregationIntoTableScan` would refuse to match across a residual `FilterNode`. Measured plans show otherwise: Trino fuses the predicate into a `ScanFilterProject`, and an aggregate over a fully-enforced `DOUBLE` range pushes straight through it. The planner is not the blocker.)* What actually declines the `BIGINT` case is §6/10, this design's own rule. That rule is therefore the only thing standing between a prefilter-only constraint and a wrong answer: a `BIGINT` range is enforced **jointly** by the pushed AQL and Trino's residual re-check, so an aggregate computed over the AQL side alone would count fractional and out-of-long-range values that the residual drops. The user-visible effect is an asymmetry — `... WHERE double_col > 100 GROUP BY city` pushes, `... WHERE bigint_col > 100 ...` does not — inherited from M2's C2/C3 resolution. A missed optimization, protecting against a wrong answer. Pinned by `ArangoConnectorAggregationTest.residualFilterInteractionIsPinned`.
