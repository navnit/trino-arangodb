...
- **Client-JDK decoupling:** the connector runs inside the Trino **server** JVM and so is built for the server's JDK (Java 23). This is independent of the JDK used by applications that query Trino — those connect over the wire via the `trino-jdbc` driver (Java 11+; not the connector), so **Java 17+ client applications are fully compatible** with a Java 23 cluster running this connector. The connector is not an embeddable library; apps wanting to embed ArangoDB access without a Trino cluster should use `arangodb-java-driver` directly instead.
