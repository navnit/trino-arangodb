...
- **`AqlSchemaOverrideAssumptionsTest`** (container-backed, house pattern of `AqlSemanticsAssumptionsTest`): `FOR d IN @@sc FILTER d.table == @t LIMIT 2 RETURN d` against an absent collection yields `errorNum == 1203` — pinning the bind-parameter-collection error shape §4.5 relies on — plus the observed forbidden-collection error shape. The parser-surface matrix from §5 (lowercase `t`, missing seconds, space separator, `1E+2`, whitespace decimal string, sub-minute offset, ±18:00 offset, far-future year — accept/reject each, pinned against JDK/Trino drift) lives in `ValueMaterializerTest` instead: those pins need no container, and they pin the exact code path the materializer executes rather than the parsers in isolation (plan-review decision, 2026-08-06).
